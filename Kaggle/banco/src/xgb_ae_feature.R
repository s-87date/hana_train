...
tr <- read_csv("../dat/train.csv") %>% select(-ID)
te <- read_csv("../dat/test.csv") %>% select(-ID)
...
read_csv("../dat/sample_submission.csv") %>%  
  mutate(target = expm1(predict(m_xgb, dtest))) %>%
  write_csv(paste0("../output/xgb_aec_", round(m_xgb$best_score, 5), ".csv"))
